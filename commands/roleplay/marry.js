	...
	run: async (client, message, args) => {
		...
		if (!mUser)
			return embed.error(
				...
				"How will you get married, if you don't tell me who do you want to marry?!",
				message
			);
		...
		await schema.findOne({ User: message.author.id }, async (err, data) => {
			...
			if (!data) {
				new schema({
					...
					.then(
						embed.embed(
							`You are now married to ${mUser.tag}!`,
							'Please tell them to run the command mentioning you so it is signed by both of you!',
							message
						)
					);
			}
