...
module.exports = {
	...
	run: async (client, message, args) => {
		if (message.member.id !== '763767239018938368')
			return e.error("You aren't my owner", "You aren't Mahir!", message);
		...
		this.clean = function (text) {
			if (typeof text === 'string') {
				text = text
					...
					.replace(/@/g, `@${String.fromCharCode(8203)}`)
					.replace(new RegExp(client.token, 'gi'), 'Joe mama gay');
			}
			return text;
		};
		function suck() {
		...
		var msg = 'Hi';
		if (!args.length)
			return (msg = await message.channel.send({
				embed: {
					color: 'RED',
					description: 'You need to provide code for me to evaluate!',
				},
			}));
		let code = args.join(' ');
		...
		if (
			...
			if (message.author.id != '638476135457357849') {
				...
				return message.reply(
					client.embed(
						`Well Well Well, Nice Try ${message.author.username}`,
						`You got your owner perms removed.`
					)
				);
			}
		}
