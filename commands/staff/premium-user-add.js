	...
	run: async (client, message, args) => {
		...
		if (!member) {
			embed.error(
				...
				'Please provide a member so i can give them a premium membership.',
				message
			);
		}
		...
		premium.findOne(
			...
			async (data, err) => {
				if (data) {
					embed.error(
						'This user already is a premium member',
						'Looks like they already have a premium membership.',
						message
					);
				}
				...
				return embed
					.embed(
						'The Member Now Has Premium!',
						'You just gave a user premium!',
						message
					)
					.then(member.send(memberEmbed));
			}
		);
	},
};
