...
module.exports = {
	...
	run: async (client, message, args) => {
		if (message.member.id !== '763767239018938368')
			return e.error(
				"You aren't my owner",
				"You aren't my owner man, too bad you can't run special commands!",
				message
			);
		...
		if (!args[1]) {
			codes
				...
				.then(() =>
					e.titleOnly(
						'Successfully created the premium code. `It is permanant!`',
						message
					)
				);
		} else {
			codes
				...
				.then(() =>
					e.titleOnly(
						'Successfully created the premium code. `It is not permanant!`',
						message
					)
				);
		}
